...
def get_vectorstore_from_url(url):
    ...
    documnet_chunks = text_splitter.split_documents(documnet)
    
    # create vectore store for each chunk
...
def get_response(user_query):
    ...
    respons = conversational_rag_chain.invoke({
        "chat_history": st.session_state.chat_history,
        "input": user_query,
    })
    return respons['answer']
...
if website_url is None or website_url == "":
    st.info("Please enter a valid article URL to be analyzed.")
else:
    # session state
    ...
    if user_query is not None and user_query != "":
        respons = get_response(user_query)
        st.session_state.chat_history.append(HumanMessage(content=user_query))
        st.session_state.chat_history.append(AIMessage(content=respons))
